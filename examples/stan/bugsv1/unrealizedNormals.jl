...
abstract type UnrealizedValue end #The base type for "values" that appear in the trace but
                #are never actually sampled (integrated out of the logpdf).

abstract type UnrealizedNormalValue <: UnrealizedValue end
                #Eventually, there would be one type like this for each conjugate pair of distributions

abstract type UnrealizedNormalLeafValue <: UnrealizedNormalValue end
                #A single normal value, created directly by a single "sampling" call (~)
...
mutable struct UnrealizedNormalNodeValue <: UnrealizedNormalValue #A linear combination of `UnrealizedNormalLeafValue`s
    offset::Float64
    components::Vector{Tuple{Float64,UnrealizedNormalLeafValue}} #should be unique
end

mutable struct ObservedNormal #Data structure for handling observables that are downstream of `UnrealizedNormalValue`s
    unrealized::UnrealizedNormalNodeValue
    SD::Float64
end

struct UnrealizedDistribution{T} <: Gen.Distribution{T} #Just a wrapper to fit the above into the Gen type hierarchy
    func
end
...
function Gen.logpdf(d::ObservedNormal, v::Float64)
    ...
    totalVariance = d.SD^2 + sum([(mul * leaf.currentCondSD)^2 for (mul,leaf) in com])
    ...
    for (i, (mul, leaf)) in enumerate(com)
        if leaf.currentCondSD > 0. && mul != 0.
            ...
            currentVariance = (mul * leaf.currentCondSD)^2

            leaf.currentCondSD = leaf.currentCondSD * sqrt( (totalVariance - currentVariance) / totalVariance)
            leaf.currentMean += delta * currentVariance / totalVariance / mul
...
#
# function Gen.logpdf(d::ObservedNormal, v::Float64) #v1: kinda sorta close, but wrong.
#     μ0, σ2, μt = meanvarmean(d)
#     lpdf = Gen.logpdf(normal, v, μt, sqrt(σ2))
#
#     #@debug "logpdf" d μ0 σ2 μt v lpdf
